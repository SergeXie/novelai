...
async def multi_completions(
        ...
        templateKey: str = Body(None, embed=True),
        gid: Optional[str] = Body(None, embed=True),
        db=Depends(get_db),
        current_user: User = Depends(get_current_user),
):
    ...
    if templateKey is not None:
        prompt_template = await PromptSquareService.get_template_by_key(db, templateKey)
        if prompt_template:
            chat_content = prompt_template.content + "\r\n" + chat_content

    await check_user_quota_or_raise(
        frozen_token_length=(len(chat_content) + 3000),
